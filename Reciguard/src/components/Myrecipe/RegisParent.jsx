...
const RegisParent = () => {
    ...
    const handleSubmit = async () => {
        try {
            ...
            // instructions 데이터 변환
            const formattedInstructions = recipeForm.instructions.map((instruction) => ({
            ...
            recipeForm.instructions.forEach((instruction, index) => {
                if (instruction.image instanceof File) {
                    payloadForm.append(`instructionImageFiles[${index + 1}]`, instruction.image);
                }
            });
            ...
            // API 요청
            const response = await fetch("http://localhost:8080/api/recipes/myrecipe/save", {
                method: "POST",
                headers: {
